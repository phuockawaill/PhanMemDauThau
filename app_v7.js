...
function exportToTxt() {
    ...
    const filterSections = document.querySelectorAll('.filter-section:not(#sidebar-menu-main)');
        ...
        if (!ds) {
            ...
            const allPkg = window.allPackages || [];
            const totalPackages = allPkg.length;
            const ongoingPackages = allPkg.filter(p => !p.completedSteps || p.completedSteps.length < (window.templates ? window.templates.length : 19)).length;
            
            // Estimate total budget if exists
            let totalBudget = 0;
            allPkg.forEach(pkg => {
                if (pkg.stepsData) {
                    for (let stepId in pkg.stepsData) {
                        for (let fieldKey in pkg.stepsData[stepId]) {
                            const val = pkg.stepsData[stepId][fieldKey];
                            if (typeof val === 'string' && (fieldKey.toLowerCase().includes('dự toán') || fieldKey.toLowerCase().includes('tổng giá trị'))) {
                                // Try to extract numbers
                                const numStr = val.replace(/[^0-9]/g, '');
                                if (numStr) {
                                    totalBudget += parseInt(numStr, 10);
                                }
                            }
                        }
                    }
            ...
            let budgetDisplay = totalBudget > 0 ? new Intl.NumberFormat('vi-VN', { style: 'currency', currency: 'VND' }).format(totalBudget) : "Đang cập nhật...";

            ds.innerHTML = `
                <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 24px;">
                    <h2 style="margin: 0; font-size: 24px; font-weight: 700; color: #1e293b;">Bảng điều hành Thống kê</h2>
                </div>

                <div style="display: grid; grid-template-columns: repeat(3, 1fr); gap: 20px; margin-bottom: 24px;">
                    <div style="background: rgba(255,255,255,0.7); backdrop-filter: blur(10px); border: 1px solid rgba(79,70,229,0.1); border-radius: 12px; padding: 24px; box-shadow: 0 4px 6px -1px rgba(79,70,229,0.05); display: flex; flex-direction: column; gap: 12px;">
                        <div style="display: flex; align-items: center; gap: 12px; color: #4f46e5;">
                            ...
                            <span style="font-weight: 600; font-size: 16px;">Số lượng Gói thầu</span>
                        </div>
                        <div style="font-size: 36px; font-weight: 700; color: #1e293b;" id="dash-total-pkg">${totalPackages}</div>
                    </div>

                    <div style="background: rgba(255,255,255,0.7); backdrop-filter: blur(10px); border: 1px solid rgba(79,70,229,0.1); border-radius: 12px; padding: 24px; box-shadow: 0 4px 6px -1px rgba(79,70,229,0.05); display: flex; flex-direction: column; gap: 12px;">
                        <div style="display: flex; align-items: center; gap: 12px; color: #f59e0b;">
                            <div style="padding: 10px; background: rgba(245,158,11,0.1); border-radius: 8px;"><i data-lucide="loader"></i></div>
                            <span style="font-weight: 600; font-size: 16px;">Đang thực hiện</span>
                        </div>
                        <div style="font-size: 36px; font-weight: 700; color: #1e293b;" id="dash-ongoing-pkg">${ongoingPackages}</div>
                    </div>

                    <div style="background: rgba(255,255,255,0.7); backdrop-filter: blur(10px); border: 1px solid rgba(79,70,229,0.1); border-radius: 12px; padding: 24px; box-shadow: 0 4px 6px -1px rgba(79,70,229,0.05); display: flex; flex-direction: column; gap: 12px;">
                        <div style="display: flex; align-items: center; gap: 12px; color: #10b981;">
                            <div style="padding: 10px; background: rgba(16,185,129,0.1); border-radius: 8px;"><i data-lucide="pie-chart"></i></div>
                            <span style="font-weight: 600; font-size: 16px;">Tổng dự toán</span>
                        </div>
                        <div style="font-size: 28px; font-weight: 700; color: #1e293b;" id="dash-total-budget">${budgetDisplay}</div>
                    </div>
                </div>
            `;
                ...
                if (pkg.stepsData) {
                    for (let stepId in pkg.stepsData) {
                        for (let fieldKey in pkg.stepsData[stepId]) {
                            ...
                            if (typeof val === 'string' && (fieldKey.toLowerCase().includes('dự toán') || fieldKey.toLowerCase().includes('tổng giá trị'))) {
                                const numStr = val.replace(/[^0-9]/g, '');
                                if (numStr) totalBudget += parseInt(numStr, 10);
                            }
                        }
                    }
                }
            ...
            const elTotal = document.getElementById('dash-total-pkg');
            const elOngoing = document.getElementById('dash-ongoing-pkg');
            const elBudget = document.getElementById('dash-total-budget');
            if(elTotal) elTotal.textContent = totalPackages;
            if(elOngoing) elOngoing.textContent = ongoingPackages;
            if(elBudget) elBudget.textContent = budgetDisplay;
        }
        return ds;
    }
